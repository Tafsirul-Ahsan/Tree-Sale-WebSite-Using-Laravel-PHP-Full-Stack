...
@foreach(Cart::getContent() as $cartitem)
    <tr>
      <td><img width="50" src="{{asset('uploads/'.$cartitem->attributes->thum_img)}}" alt=""></td>
      <td>{{$cartitem->attributes->code}}</td>
      <td>{{$cartitem->name}}</td>
      <td>{{$cartitem->price}}</td>
      <td>{{$cartitem->quantity}}</td>
      <td>{{$cartitem->attributes->subtotal}}</td>
      <td><a href="{{url('cart/remove-item/'.$cartitem->id)}}" class="text-danger">Remove</a></td>
    </tr>
@endforeach

    <tr>
      <td colspan="4"><a href="{{url('/')}}" style="font-size: 22px;font-weight: 500;color: blue">Buy more</a></td>
